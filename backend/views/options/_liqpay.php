        ...
        <div class="control-group ">
            ...
            <?php if(!$option->i18n) { ?>
                ...
                <div class="controls">
                    ...
                    {
                        case 'dropDown':
                            if($option->key === 'order.payment.liqpay.status.confirm')
                                $data = OrderStatus::model()->listData();
                            else
                                $data = $option->cfg(null, 'option');
                            echo CHtml::dropDownList("Options[{$option->key}]", $option->value, (array)$data);
                        break;
                        case 'textArea':
