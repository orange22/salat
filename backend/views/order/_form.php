...
<div class="row">
    ...
    <div class="span6">
        <div data-block="sjstpl" class="control-group control-group-multiple product-info">
            ...
            <!--<label for="Dish_0_product_id" class="control-label"><?php echo Yii::t('backend', 'Dish'); ?></label>-->

            <?php $OrderProduct = array_pad((array)$model->orderProducts, 1, new OrderProduct()); ?>
            <?php $OrderProductCount = count($OrderProduct); ?>
            <?php foreach($OrderProduct as $i => $item) { /** @var $item ProductInfo */?>

                <div class="controls controls-multiple-row">
                    <?php echo $form->dropDownList($item, "[{$i}]product_id", Prod::model()->sort('status DESC')->listData(), array(
                    ...
                    <div class="arrayControls">
                        <a href="#" class="btn btn-mini btnRemoveArrayRow"
                            <?php /*if($OrderProductCount == 1) { ?> style="display: none;"<?php } */?>>-</a>
                        <a href="#" class="btn btn-mini btnArrayControl btnAddArrayRow"
                            <?php if($i < $OrderProductCount - 1) { ?> style="display: none;"<?php }?>
...
<script type="text/html" id="sjstpl">
    <?php echo $form->dropDownList($OrderProduct[0], "[<%=idx%>]product_id", Prod::model()->sort('status DESC')->listData(), array(
        'quantity' => '',
        'encode' => false,
        'class' => 'span6 validate-unique',
        'placeholder' => Yii::t('backend', 'Dish'),
    ...
    <?php echo $form->textField($OrderProduct[0], '[<%=idx%>]quantity', array(
        'quantity' => '',
        'encode' => false,
        'class' => 'span3',
        'placeholder' => Yii::t('backend', 'Quantity'),
    ...
    <?php echo $form->error($OrderProduct[0], '[<%=idx%>]product_id', array('encode' => false)); ?>
    <?php echo $form->error($OrderProduct[0], '[<%=idx%>]quantity', array('encode' => false)); ?>
</script>
