 ...
 * AdminView
 */
class AdminView extends TbExtendedGridView
{
    /**
     * Grid action buttons
     *
    ...
    protected function prepareColumns()
    {
        ...
        $buts='{update} {clone} {delete} {pdf}';
        else
        $buts='{update} {clone} {delete}';
                ...
                'buttons' => array(
                    ...
                    'delete' => array(
                     ...
                     'pdf' => array(
                        ...
                        'imageUrl' => '/backend/img/pdf.png',
                        'url' => "Yii::app()->getController()->createUrl('receipt', array('{$this->keyField}' => \$data->{$this->keyField}))",
    ...
    protected function renderActions()
    {
		...
		if($this->controller->getId()=='order' || in_array('delivered', $this->actionButtons) && ($accessCtrl || Yii::app()->user->checkAccess($perm.'Update')))
        {
            
            echo CHtml::linkButton(Yii::t('backend', 'New'), array(
                           'class' => 'btn btn-info btnBulk btnDisable',
                           'submit' => $ctrl->createUrl('BulkNew'),
                       ));
           	echo CHtml::linkButton(Yii::t('backend', 'Delivered'), array(
                           'class' => 'btn  btn-success btnBulk btnEnable',
                           'submit' => $ctrl->createUrl('BulkDelivered'),
                       ));
			echo "&nbsp;&nbsp;&nbsp;";		   
        }
