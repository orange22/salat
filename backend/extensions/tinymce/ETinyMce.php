...
class ETinyMce extends CInputWidget
{
   ...
   *
   ...
   protected function makeFullEditor($url='')
   {
      ...
      $this->plugins = array_merge(array('safari','pagebreak','style','layer','table','save','advhr','advimage','advlink','emotions','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','contextmenu','paste','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template'), $this->plugins);
      ...
      if ($this->useElFinder) {
          $options['file_browser_callback'] = 'js:function(field_name, url, type, win) {
            ...
            if($("#finder").length == 0) {
                ...
                $("#finder").elfinder({ zIndex: 9999999, places: "Избранное", url : "'.Yii::app()->urlManager->createUrl('elfinder/default/connector').'",
                ...
                //$("#finder").elfinder({ zIndex: 9999999, places: "Избранное", url : "'.Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets').'/elfinder/connectors/connector.php?url='.Yii::app()->baseUrl.'/upload'.'",
                    ...
                    scriptData:{'.Yii::app()->request->csrfTokenName.':"'.Yii::app()->request->getCsrfToken().'"}
                });
            }
            else {
